...
function setupEventListeners() {
    ...
    // Yönetim Paneli butonu artık admin/admin.html'e yönlendiriyor.
...
// ... Kalan tüm fonksiyonlarınız (initiateDeleteItem, addProductToList, vb.) burada değişmeden kalır ...
// ... Bu fonksiyonlar Denetim Formu'na ait olduğu için temizlenmemiştir ...
        ...
        if (q.type === 'standard') {
            ...
            if (itemsForEmail.length > 0) {
                itemsForEmail.sort((a, b) => {
                    ...
                    if (aIsLink && !bIsLink) return 1; if (!aIsLink && bIsLink) return -1;
                    return 0;
                });
...
function updateFormInteractivity(enable) {
    ...
    const buttons = formContent.querySelectorAll('.add-item-btn, .status-btn, .remove-btn, .delete-bar, .delete-item-btn, .product-adder button');
    const inputs = formContent.querySelectorAll('#product-selector, #product-qty');
