...
async function loadInitialData() {
    ...
    // 2. Bulut başarısız olursa veya kullanıcı giriş yapmamışsa, yerel cache'i dene
    if (!questionsLoaded) {
        const cachedQuestions = localStorage.getItem('fideQuestionsCache');
        if (cachedQuestions) {
            try {
                const cachedData = JSON.parse(cachedQuestions);
                fideQuestions = cachedData.questions || [];
                productList = cachedData.productList || [];
                console.log("Sorular ve ürün listesi yerel önbellekten yüklendi.");
                questionsLoaded = true;
                localDataUsed = true;
            } catch (e) {
                 console.error("Yerel soru önbelleği okunamadı:", e);
            }
        }
    }

    // 3. Yerel cache de boşsa, son çare olarak JSON dosyasını dene
    if (!questionsLoaded) {
         try {
            ...
            localStorage.setItem('fideQuestionsCache', JSON.stringify(jsonData)); // İlk yüklemede cache'i doldur
    ...
    // Eğer başlangıçta yerel veri kullanıldıysa, arka planda buluttan sessiz bir güncelleme yapmayı dene
    if (localDataUsed && auth.currentUser && database) {
        database.ref('fideQuestionsData').once('value').then(snapshot => {
            if (snapshot.exists()) {
                 const cloudData = snapshot.val();
                 const localCache = localStorage.getItem('fideQuestionsCache');
                 if (JSON.stringify(cloudData) !== localCache) {
                     localStorage.setItem('fideQuestionsCache', JSON.stringify(cloudData));
                     console.log("Arka planda daha güncel soru listesi bulundu ve yerel önbellek güncellendi. Sayfayı yenilemek en son veriyi gösterecektir.");
                 }
            }
        });
    }
...
async function loadExcelData() {
    // DiDe verisini yükle: Önce yerel, sonra bulut
    loadDideDataFromStorage();
    if (auth.currentUser && database) {
        const dideRef = database.ref('excelData/dide');
        dideRef.once('value').then(snapshot => {
            if (snapshot.exists()) {
                const storedData = snapshot.val();
                const ageInDays = (new Date().getTime() - storedData.timestamp) / (1000 * 60 * 60 * 24);
                if (ageInDays <= 30) {
                    localStorage.setItem('didePersistenceData', JSON.stringify(storedData));
                    if (JSON.stringify(dideData) !== JSON.stringify(storedData.data)) {
                        populateDideState(storedData.data);
                        if (storedData.filename) { document.getElementById('file-name').textContent = `Buluttan güncellendi: ${storedData.filename}`; }
                    }
                }
            }
        });
    ...
    // FiDe verisini yükle: Önce yerel, sonra bulut
    loadFideDataFromStorage();
    if (auth.currentUser && database) {
        const fideRef = database.ref('excelData/fide');
        fideRef.once('value').then(snapshot => {
            if (snapshot.exists()) {
                const storedData = snapshot.val();
                const ageInDays = (new Date().getTime() - storedData.timestamp) / (1000 * 60 * 60 * 24);
                if (ageInDays <= 30) {
                    localStorage.setItem('fidePersistenceData', JSON.stringify(storedData));
                     if (JSON.stringify(fideData) !== JSON.stringify(storedData.data)) {
                        populateFideState(storedData.data);
                        if (storedData.filename) { document.getElementById('fide-file-name').textContent = `Buluttan güncellendi: ${storedData.filename}`; }
                    }
                }
            }
        });
...
// --- Excel Kütüphanesini Sadece Gerektiğinde Yükleme ---
function ensureXlsxLibrary(callback) {
    if (isXlsxLibraryLoaded) {
    ...
    script.onerror = () => {
        ...
        console.error('XLSX library could not be loaded.');
    };
