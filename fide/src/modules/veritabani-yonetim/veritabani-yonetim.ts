...
/* fide/modules/veritabani-yonetim/veritabani-yonetim.ts */

export async function initializeVeritabaniYonetimModule(pb: PocketBase) {
...
function getEl<T extends HTMLElement>(id: string): T {
    ...
    if (!el) throw new Error(`Beklenen element bulunamadı: #${id}`);
    return el as T;
}
...
/**
 * Ortak Modal Penceresi (2026: hidden ile yönetilir)
 */
function showCommonModal(title: string, bodyHtml: string, footerHtml: string) {
    const modal = document.getElementById('db-common-modal') as HTMLElement | null;
    const titleEl = document.getElementById('modal-title') as HTMLElement | null;
    const bodyEl = document.getElementById('modal-body') as HTMLElement | null;
    const footerEl = document.getElementById('modal-footer') as HTMLElement | null;
    if (!modal || !titleEl || !bodyEl || !footerEl) {
        throw new Error('Modal DOM elemanları bulunamadı (db-common-modal / modal-title / modal-body / modal-footer).');
    }
...
function setupEventListeners(pb) {
    getEl<HTMLButtonElement>('btn-refresh-stats').addEventListener('click', () => void loadStats(pb));
    getEl<HTMLButtonElement>('btn-close-modal').addEventListener('click', closeCommonModal);
// Modal içi butonlar (inline onclick yok)
getEl<HTMLElement>('modal-footer').addEventListener('click', (ev) => {
    const target = ev.target as HTMLElement;
    const btn = target.closest<HTMLElement>('[data-action]');
    if (!btn) return;
    const action = btn.getAttribute('data-action');
    if (action === 'close-modal') closeCommonModal();
});


    // --- GELİŞMİŞ TEMİZLİK & BAKIM FONKSİYONU ---
    getEl<HTMLButtonElement>('btn-action-clean-maintenance').addEventListener('click', async () => {
        showCommonModal('Temizlik & Bakım Analizi', 
            ...
            '<button class="btn-secondary btn-sm" data-action="close-modal">Vazgeç</button>');
        ...
        try {
            ...
            if (idsToDelete.length === 0) {
                ...
                getEl<HTMLButtonElement>('modal-btn-execute-cleanup').addEventListener('click', async () => {
                    const execBtn = getEl<HTMLButtonElement>('modal-btn-execute-cleanup');
    ...
    getEl<HTMLButtonElement>('btn-action-user-delete').addEventListener('click', async () => {
        ...
        `;
        showCommonModal('Kullanıcı Silme & Veri Yönetimi', bodyHtml, `<button class="btn-secondary btn-sm" data-action="close-modal">Vazgeç</button><button id="modal-btn-execute" class="btn-danger btn-sm" disabled>İşlemi Onayla</button>`);

        const selectUser = getEl<HTMLSelectElement>('modal-select-user');
        const executeBtn = getEl<HTMLButtonElement>('modal-btn-execute');

        selectUser.addEventListener('change', async () => {
            const userId = selectUser.value;
            const analysisArea = getEl<HTMLElement>('modal-analysis-area');
            const strategyArea = getEl<HTMLElement>('modal-strategy-area');
            if (!userId) {
                setHidden(strategyArea, true);
                setHidden(analysisArea, true);
                executeBtn.disabled = true;
                return;
            }
            analysisArea.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Analiz ediliyor...';
            setHidden(analysisArea, false);
            ...
            setHidden(strategyArea, false);
            executeBtn.disabled = false;
        });

        getEl<HTMLElement>('modal-body').addEventListener('click', (e) => {
            const radio = e.target.closest('.db-card-option')?.querySelector('input');
            if (radio && radio.name === 'del-strat') { const t = getEl<HTMLElement>('modal-transfer-select'); setHidden(t, radio.value !== 'transfer'); }
        });
    ...
    getEl<HTMLButtonElement>('btn-export-settings').addEventListener('click', async () => {
        try {
            const bodyHtml = `
            ...
            getEl<HTMLButtonElement>('modal-btn-export-execute').addEventListener('click', async () => {
...
for (const col of selectedCols) {
    ...
    getEl<HTMLInputElement>('input-import-settings').addEventListener('change', async (e) => {
        ...
        reader.onload = async (ev) => {
            try {
                ...
                const footerHtml = `<button class="btn-secondary btn-sm" data-action="close-modal">Vazgeç</button><button id="btn-res-exec" class="btn-primary btn-sm">Geri Yüklemeyi Başlat</button>`;
                showCommonModal('Yedekten Geri Yükle', bodyHtml, footerHtml);

                getEl<HTMLButtonElement>('btn-res-exec').addEventListener('click', async () => {
    ...
    getEl<HTMLButtonElement>('btn-action-reset-mappings').addEventListener('click', async () => {
    ...
    getEl<HTMLButtonElement>('btn-clear-excel').addEventListener('click', () => { if(confirm("Emin misiniz?")) clearCollection(pb, 'excel_verileri', "Excel"); });
    getEl<HTMLButtonElement>('btn-clear-reports').addEventListener('click', () => { if(confirm("SİL yazın") && prompt("Onay") === "SİL") clearCollection(pb, 'denetim_raporlari', "Raporlar"); });
    getEl<HTMLButtonElement>('btn-clear-undone').addEventListener('click', () => { if(confirm("Temizlensin mi?")) clearCollection(pb, 'denetim_geri_alinanlar', "Log"); });
}
