 ...
 */
export function initializeKullaniciYoneticisiModule(pbInstance: PocketBase) {
    
    // --- Global Değişkenler ve DOM Elementleri ---
    const pb = pbInstance;
    let allUsersCache = []; 

    // --- Görünürlük Yardımcıları (2026 modern: hidden attribute) ---
    const setHidden = (el: Element | null, hidden: boolean) => {
        if (el instanceof HTMLElement) el.hidden = hidden;
    };
    const show = (el: Element | null) => setHidden(el, false);
    const hide = (el: Element | null) => setHidden(el, true);
    ...
    async function loadUsers() {
        ...
        try {
            ...
            if (!listView.hidden) { renderUsersTable(allUsersCache); }
        } catch (error) {
            console.error('Hata:', error);
            if (!listView.hidden) { tableBody.innerHTML = '<tr><td colspan="7" style="text-align: center; color: red;">Yüklenemedi.</td></tr>'; }
        }
    }
    ...
    function showFormView() { hide(listView); show(formView); }
    function showListView() { hide(formView); show(listView); renderUsersTable(allUsersCache); }

    // --- 4. CRUD Fonksiyonları ---
    function handleNew() {
        form.reset(); userIdInput.value = ''; formTitle.textContent = 'Yeni Kullanıcı Ekle';
        show(passwordWrapper); userEmailInput.disabled = false;
        hide(userBanSection); hide(devicesHr);
        show(userDeviceLimitSection); userDeviceLimitInput.value = 1;
        showFormView();
    }
    ...
    function handleEdit(userId) {
        ...
        setHidden(userBanSection, user.role === 'admin');
        
        updateBanButton(user.is_banned); show(devicesHr);
        if (user.role === 'client') { loadUserDevices(user.id); show(userDeviceLimitSection); userDeviceLimitInput.value = user.device_limit || 1; }
        else { hide(userDeviceLimitSection); hide(userDevicesTableWrapper); }
        showFormView();
    }
    ...
    userRoleSelect.addEventListener('change', () => { setHidden(userDeviceLimitSection, userRoleSelect.value !== 'client'); });
