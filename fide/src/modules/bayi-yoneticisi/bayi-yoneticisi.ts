 ...
 */
export async function initializeBayiYoneticisiModule(pbInstance: PocketBase) {
    ...
    const closeImportModalFn = () => { if(importModal) hide(importModal); };
    ...
    function showBulkAssignLoading(s, t) {
        ...
        setHidden(bulkAssignLoadingOverlay, !s);
    }

    function showLoading(show) {
        setHidden(loadingSpinner, !show);
    }

    // --- OLAY DİNLEYİCİLERİ ---
    container.querySelector('#btn-yeni-bayi').addEventListener('click', () => { 
        bayiForm.reset(); bayiIdInput.value = ''; modalTitle.textContent = 'Yeni Bayi Ekle'; show(modal); 
    });
    container.querySelector('#btn-modal-cancel').addEventListener('click', () => hide(modal));
    ...
    btnBulkAssignCancel.addEventListener('click', () => hide(bulkAssignModal));
