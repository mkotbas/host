...
// --- TABLO STİLLERİ (Merkezi Yönetim) ---
const TABLE_STYLES = {
    container: 'overflow-x: auto; margin: 20px 0;',
    table: 'border-collapse: collapse; width: auto; font-family: Aptos, Arial, sans-serif; font-size: 9pt;',
    
    headerCell: 'border: 1px solid #000; padding: 8px 6px; text-align: center; font-weight: bold; background-color: #ff0000; color: #000; font-family: Aptos, Arial, sans-serif; font-size: 9pt;',
    headerYear: 'border: 1px solid #000; padding: 8px 6px; text-align: center; font-weight: bold; background-color: #ff0000; color: #000; font-family: Aptos, Arial, sans-serif; font-size: 9pt;',
    headerMonth: 'border: 1px solid #000; padding: 8px 6px; text-align: center; font-weight: bold; background-color: #ff0000; color: #000; white-space: nowrap; font-family: Aptos, Arial, sans-serif; font-size: 9pt;',
    headerAverage: 'border: 1px solid #000; padding: 8px 6px; text-align: center; font-weight: bold; background-color: #ff0000; color: #000; font-family: Aptos, Arial, sans-serif; font-size: 9pt;',
    
    labelCell: 'border: 1px solid #000; padding: 8px 6px; text-align: center; font-weight: bold; background-color: #ff0000; color: #000; font-family: Aptos, Arial, sans-serif; font-size: 9pt;',
    scoreCell: 'border: 1px solid #000; padding: 8px 6px; text-align: center; font-family: Aptos, Arial, sans-serif; font-size: 10pt;',
    averageCell: 'border: 1px solid #000; padding: 8px 6px; text-align: center; font-weight: bold; font-family: Aptos, Arial, sans-serif; font-size: 9pt;'
};
...
function renderPerformanceTable(storeInfo, fideStoreInfo, manualFideScore) {
    ...
    let mHeaders = "";
    for (let i = 1; i <= 12; i++) {
        mHeaders += `<th style="${TABLE_STYLES.headerMonth}">${state.monthNames[i].toUpperCase()}</th>`;
    }
    mHeaders += `<th style="${TABLE_STYLES.headerAverage}"><div>YIL</div><div>ORTALAMASI</div></th>`;

    let dScores = "";
    for (let i = 1; i <= 12; i++) {
        const score = storeInfo?.scores?.[i];
        // Boş puan kontrolü - varsa değeri göster, yoksa "-" koy
        const displayValue = (score !== undefined && score !== null && score !== "") ? score : "-";
        dScores += `<td style="${TABLE_STYLES.scoreCell}">${displayValue}</td>`;
    }
    dScores += `<td style="${TABLE_STYLES.averageCell}">${calculateAverage(storeInfo?.scores, currentMonthIdx)}</td>`;

    let fScores = "";
    for (let i = 1; i <= 12; i++) {
        let displayVal = fideStoreInfo?.scores?.[i];
        
        // Cari ay kontrolü (manuel girilen puan varsa)
        if (i === currentMonthIdx && manualNum !== null && !isNaN(manualNum) && (!displayVal || displayVal === "")) {
            displayVal = manualFideScore;
        }
        
        // Boş puan kontrolü - varsa değeri göster, yoksa "-" koy
        const finalDisplay = (displayVal !== undefined && displayVal !== null && displayVal !== "") ? displayVal : "-";
        
        fScores += `<td style="${TABLE_STYLES.scoreCell}">${finalDisplay}</td>`;
    }
    fScores += `<td style="${TABLE_STYLES.averageCell}">${calculateAverage(fideStoreInfo?.scores, currentMonthIdx, manualNum)}</td>`;
    ...
    return `
        <div style="${TABLE_STYLES.container}">
            <table style="${TABLE_STYLES.table}">
                <thead>
                    <tr>
                        <th style="${TABLE_STYLES.headerYear}">${cYear}</th>
                        ${mHeaders}
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td style="${TABLE_STYLES.labelCell}">DİDE</td>
                        ${dScores}
                    </tr>
                    <tr>
                        <td style="${TABLE_STYLES.labelCell}">FİDE</td>
                        ${fScores}
                    </tr>
                </tbody>
            </table>
    ...
    state.fideQuestions.forEach(q => {
        ...
        if (q.type === 'standard') {
            ...
            if (container) {
                ...
                let emailItems = hasDyn ? items : items.filter(i => i.type === 'static');
