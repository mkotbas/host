...
async function applyDeleteQuestionScenario() {
    ...
    if (!confirm(`DİKKAT! BU İŞLEM GERİ ALINAMAZ!\n\nID: ${question.id}\nSoru: "${question.title}"\n\nYukarıdaki soruyu ve bu soruya ait TÜM bayi raporlarındaki cevapları kalıcı olarak silmek istediğinizden KESİNLİKLE emin misiniz?`)) { alert("İşlem iptal edildi."); return; }
...
function renderMigrationManagerUI() {
    ...
    if (Object.keys(migrationMap).length === 0) { listContainer.innerHTML = '<li class="empty-message">Henüz yönlendirme eklenmemiş.</li>'; }
        ...
        for (const oldId in migrationMap) {
            ...
            listItem.innerHTML = `<span class="mapping-text">Eski ID: <b>${oldId}</b> <i class="fas fa-long-arrow-alt-right"></i> Yeni ID: <b>${newId}</b></span><button class="btn-danger btn-sm" onclick="deleteMigrationMapping('${oldId}')" title="Bu yönlendirmeyi sil."><i class="fas fa-trash"></i></button>`;
            listContainer.appendChild(listItem);
        }
    }
}
async function addMigrationMapping(oldIdValue, newIdValue) {
    if (!oldIdValue || !newIdValue) { alert("Lütfen hem 'Eski ID' hem de 'Yeni ID' alanlarını doldurun."); return; }
    if (oldIdValue === newIdValue) { alert("Eski ve yeni ID aynı olamaz."); return; }
...
async function saveMigrationMap() {
    if (auth.currentUser && database) {
        ...
        catch (error) { console.error("Veri taşıma ayarları buluta kaydedilemedi:", error); alert("Hata: Yönlendirme ayarları buluta kaydedilemedi."); }
    }
}
    ...
    fideQuestions.sort((a, b) => a.id - b.id).forEach(q => {
        ...
        itemDiv.innerHTML = `
            <div class="manager-item-grid">
                <div><label>Soru ID</label><input type="number" class="manager-id-input" value="${q.id}" disabled title="ID değiştirmek veri bütünlüğünü bozabilir. Düzenlemek için şifre gerekir."></div>
            ...
            <div class="manager-item-footer"><button class="btn-warning btn-sm" onclick="deleteAllAnswersForQuestion(${q.id})" title="Bu soruya ait TÜM cevapları BÜTÜN bayi raporlarından kalıcı olarak siler."><i class="fas fa-eraser"></i>Cevapları Temizle</button></div>`;
...
function renderPopManagerUI(container, questionData) {
    ...
    container.innerHTML = `
        <p class="pop-manager-info"><i class="fas fa-info-circle"></i> Kodları aralarına virgül (,) koyarak girin. Boşluklar otomatik olarak temizlenecektir...</p>
        <div class="pop-manager-group"><label for="pop-codes-input-${questionData.id}">Geçerli POP Kodları</label><textarea id="pop-codes-input-${questionData.id}" class="pop-codes-input" rows="5" placeholder="100001, 100002...">${popCodes}</textarea></div>
        <div class="pop-manager-group"><label for="expired-pop-codes-input-${questionData.id}">Süresi Dolmuş (Biten) POP Kodları</label><textarea id="expired-pop-codes-input-${questionData.id}" class="expired-pop-codes-input" rows="3" placeholder="900001, 900002...">${expiredCodes}</textarea></div>`;
}
function renderProductManagerUI(container) {
    ...
    container.innerHTML = `
        ...
        <div class="bulk-add-container">
            ...
            <p class="bulk-add-info">Her satıra bir ürün gelecek şekilde yapıştırın...</p>
            <div class="bulk-add-controls">
                ...
                <textarea id="bulk-product-input" placeholder="88001 Siyah T-Shirt..."></textarea>
            </div>
...
function toggleDetailedEditor(button) {
    ...
    if (panel.classList.contains('open')) { button.innerHTML = '<i class="fas fa-eye-slash"></i> Detaylı Liste Editörünü Gizle'; }
    else { button.innerHTML = '<i class="fas fa-edit"></i> Detaylı Liste Editörünü Göster'; }
}
function parseAndAddProducts() {
    ...
    } else { alert("Hiçbir ürün eklenemedi. Lütfen formatı kontrol edin (Stok Kodu BOŞLUK Ürün Adı)."); }
}
function addCategoryRow(container, category = {}, targetElement = null) {
    ...
    row.draggable = true;
    row.innerHTML = `<i class="fas fa-grip-vertical drag-handle" title="Sıralamak için sürükleyin"></i><i class="fas fa-tag category-icon"></i><input type="text" placeholder="Kategori Adı" value="${category.name || ''}"><button class="btn-danger btn-sm" onclick="this.parentElement.remove()"><i class="fas fa-trash"></i></button>`;
...
function addProductRow(container, product = {}, targetElement = null) {
    ...
    row.draggable = true;
    row.innerHTML = `<i class="fas fa-grip-vertical drag-handle" title="Sıralamak için sürükleyin"></i><input type="text" class="product-code" placeholder="Stok Kodu" value="${product.code || ''}"><input type="text" class="product-name" placeholder="Ürün Adı" value="${product.name || ''}"><button class="btn-danger btn-sm" onclick="this.parentElement.remove()"><i class="fas fa-trash"></i></button>`;
    if (targetElement) { container.insertBefore(row, targetElement.nextSibling); }
    else { container.appendChild(row); }
    return row;
}
...
function deleteAllArchivedQuestions() {
    ...
    if (confirm(`Arşivdeki ${itemsToDelete.length} sorunun tümünü kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.`)) {
        itemsToDelete.forEach(item => {
            ...
            setTimeout(() => { item.style.display = 'none'; item.classList.add('to-be-deleted'); }, 500);
        });
...
async function deleteAllAnswersForQuestion(questionId) {
    ...
    if (!confirm(`DİKKAT! Bu işlem geri alınamaz.\n\nFiDe ${questionId} ("${questionTitle}") sorusuna ait TÜM cevapları, BÜTÜN bayi raporlarından kalıcı olarak silmek istediğinizden emin misiniz?`)) { alert("İşlem iptal edildi."); return; }
    ...
    } catch (error) { console.error("Cevapları silerken bir hata oluştu:", error); alert("Bir hata oluştu! Cevaplar silinemedi. Lütfen konsolu kontrol edin."); }
    finally { loadingOverlay.style.display = 'none'; }
}
async function saveQuestions() {
    if (!auth.currentUser || !database) { alert("Değişiklikleri buluta kaydetmek için lütfen giriş yapın."); return; }
