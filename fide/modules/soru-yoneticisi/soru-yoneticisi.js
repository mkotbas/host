...
// --- YENİ (VERİ KORUMA) ---
// Buluttan okunan styling verisini (4 tablo) korumak için
let stylingData = {}; 
...
async function loadInitialData() {
    ...
    try {
        ...
        // --- YENİ (VERİ OKUMA VE KORUMA) ---
        // Buluttaki 'stylingData' verisini oku ve korumak için değişkene ata
        stylingData = cloudData.stylingData || {}; 
        
        // --- YENİ (FiDe 16 TİPİNİ GÜNCELLEME) ---
        // FiDe 16 sorusunu bul ve tipini 'styling_list' olarak zorla
        // Bu sayede arayüz (UI) doğru yüklenir.
        const fide16Soru = fideQuestions.find(q => q.id === 16);
        if (fide16Soru && fide16Soru.type !== 'styling_list') {
            fide16Soru.type = 'styling_list';
            // Not: Bu sadece hafızada değişir, "Kaydet" butonuna basınca kalıcı olur.
        }
...
async function saveQuestions(reloadPage = true) {
    ...
    // --- GÜNCELLENDİ (VERİ KORUMA) ---
    // 'stylingData' verisini (4 tablo) dokunmadan geri kaydet
    const finalJsonData = { 
        questions: newQuestions, 
        productList: newProductList,
        stylingData: stylingData // Hafızadaki (buluttan okunan) veriyi geri ekle
    };
    // --- GÜNCELLEME BİTTİ ---
...
function renderQuestionManager(){ const m=document.getElementById('manager-list'); if(!m)return; m.innerHTML=''; fideQuestions.sort((a,b)=>a.id-b.id).forEach(q=>{const d=document.createElement('div'); d.className='manager-item'; d.dataset.id=q.id; let s=(q.staticItems||[]).join('<br>'); 
    ...
    // --- YENİ (STYLING LİSTESİ) ---
    // 'styling_list' tipini dropdown listesine ekle
    const t=['standard','product_list', 'styling_list', 'pop_system']; 
    // --- YENİ BİTTİ ---
...
function addNewQuestionUI(){ if(currentManagerView!=='active'){return;} const m=document.getElementById('manager-list'); const e=Array.from(m.querySelectorAll('.manager-id-input')).map(i=>parseInt(i.value)); const n=e.length>0?Math.max(...e)+1:1; const d=document.createElement('div'); d.className='manager-item'; d.style.backgroundColor='#dcfce7'; d.dataset.id=n; 
    ...
    // --- YENİ (STYLING LİSTESİ) ---
    // 'styling_list' tipini dropdown listesine ekle
    /* GÜNCELLENDİ: onclick ve onchange eventleri HTML'den kaldırıldı */ 
    d.innerHTML=`<div class="manager-item-grid"><div><label>Soru ID</label><input type="number" class="manager-id-input" value="${n}"></div><div><label>Soru Başlığı</label><input type="text" class="question-title-input" placeholder="Yeni soru..."></div><div><label>Soru Tipi</label><select class="question-type-select"><option value="standard" selected>standard</option><option value="product_list">product_list</option><option value="styling_list">styling_list</option><option value="pop_system">pop_system</option></select></div><div><label>Cevap Tipi</label><select class="answer-type-select"><option value="variable" selected>Değişken</option><option value="fixed">Sabit</option></select></div><div class="manager-grid-switch-group"><div class="archive-switch-container"><label>E-posta Ekle</label><label class="switch"><input type="checkbox" class="wants-email-checkbox"><span class="slider green"></span></label></div><div class="archive-switch-container"><label>Arşivle</label><label class="switch"><input type="checkbox" class="archive-checkbox"><span class="slider"></span></label></div></div></div><div><label>Statik Maddeler</label><div class="editor-toolbar">...</div><div class="editable-textarea" contenteditable="true"></div></div><div class="special-manager-container"></div><div class="manager-item-footer"><button class="btn-sm btn-cancel-new-question"><i class="fas fa-times"></i> İptal</button></div>`; 
    // --- YENİ BİTTİ ---
    
    m.appendChild(d); /* GÜNCELLENDİ: Event listener'lar burada ekleniyor */ d.querySelector('.question-type-select').addEventListener('change', (e_select) => toggleSpecialManagerUI(e_select.currentTarget)); d.querySelector('.archive-checkbox').addEventListener('change', filterManagerView); d.querySelector('.btn-cancel-new-question').addEventListener('click', (e_btn) => e_btn.currentTarget.closest('.manager-item').remove()); d.querySelector('input[type="text"]').focus();}
