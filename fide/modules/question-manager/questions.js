...
// --- ANA BAŞLATICI ---
// Bu fonksiyon, admin.js tarafından bu script yüklendiğinde çalışacak.
async function initializeQuestionManager() {
    if (typeof auth === 'undefined' || !auth.currentUser) {
        alert("Soru yöneticisini kullanmak için giriş yapmış olmalısınız.");
        document.getElementById('module-content').innerHTML = '<p>Lütfen önce giriş yapın.</p>';
        return;
    }
...
function setupQuestionManagerEventListeners() {
    if (document.getElementById('question-manager').dataset.listenersAttached) return;
    document.getElementById('question-manager').dataset.listenersAttached = 'true';
...
// ... Diğer tüm fonksiyonlarınız (openScenarioSystem, saveQuestions, vb.) burada değişmeden kalacak ...
// ... Aşağıya sadece değişen veya eklenen fonksiyonları değil, dosyanın tamamını koyuyorum ...
    ...
    fideQuestions.sort((a, b) => a.id - b.id).forEach(q => {
        ...
        itemDiv.innerHTML = `
            ...
            <div>
                ...
                <div class="editor-toolbar"><button onclick="formatText(this, 'bold')" title="Kalın"><i class="fas fa-bold"></i></button><button onclick="formatText(this, 'italic')" title="İtalik"><i class="fas fa-italic"></i></button><button onclick="formatText(this, 'underline')" title="Altı Çizili"><i class="fas fa-underline"></i></button><button onclick="formatText(this, 'link')" title="Köprü Ekle/Düzenle/Kaldır"><i class="fas fa-link"></i></button></div>
...
function toggleSpecialManagerUI(selectElement) {
    ...
    if (selectElement.value === 'product_list') {
        specialContainer.className = 'special-manager-container product-list-manager';
        renderProductManagerUI(specialContainer);
    } else if (selectElement.value === 'pop_system') {
        specialContainer.className = 'special-manager-container pop-manager-container';
...
function renderPopManagerUI(container, questionData) {
    ...
    container.innerHTML = `
        ...
        <div class="pop-manager-group"><label>Süresi Dolmuş (Biten) POP Kodları</label><textarea class="expired-pop-codes-input" rows="3">${expiredCodes}</textarea></div>`;
}
...
function renderProductManagerUI(container) {
    ...
    container.innerHTML = `
        ...
        <div class="bulk-add-container">
            ...
            <p class="bulk-add-info">Her satıra bir ürün gelecek şekilde (Stok Kodu BOŞLUK Ürün Adı) formatında yapıştırın.</p>
            <div class="bulk-add-controls"><select id="bulk-add-category-select">${categoryOptions}</select><textarea id="bulk-product-input" placeholder="..."></textarea></div>
        ...
        <button id="toggle-detailed-editor-btn" class="btn-sm" onclick="toggleDetailedEditor(this)"><i class="fas fa-edit"></i> Detaylı Liste Editörünü Göster</button>
        <div id="detailed-editor-panel">
            <div class="product-manager-actions"><button class="btn-primary btn-sm" onclick="addCategoryRow(this.closest('#detailed-editor-panel').querySelector('.product-list-editor'))"><i class="fas fa-tags"></i> Kategori Ekle</button><button class="btn-success btn-sm" onclick="addProductRow(this.closest('#detailed-editor-panel').querySelector('.product-list-editor'))"><i class="fas fa-box"></i> Ürün Ekle</button></div>
...
function toggleDetailedEditor(button) {
    ...
    button.innerHTML = panel.classList.contains('open') ? '<i class="fas fa-eye-slash"></i> Detaylı Liste Editörünü Gizle' : '<i class="fas fa-edit"></i> Detaylı Liste Editörünü Göster';
}
...
function parseAndAddProducts() {
    ...
    if (addedCount > 0) {
        ...
        textarea.value = '';
        const panel = document.getElementById('detailed-editor-panel');
        if (panel && !panel.classList.contains('open')) document.getElementById('toggle-detailed-editor-btn').click();
    } else {
...
function addCategoryRow(container, category = {}, targetElement = null) {
    ...
    row.className = 'category-manager-row'; row.dataset.type = 'category'; row.draggable = true;
...
function addProductRow(container, product = {}, targetElement = null) {
    const row = document.createElement('div');
    row.className = 'product-manager-row'; row.dataset.type = 'product'; row.draggable = true;
    row.innerHTML = `<i class="fas fa-grip-vertical drag-handle"></i><input type="text" class="product-code" value="${product.code || ''}"><input type="text" class="product-name" value="${product.name || ''}"><button class="btn-danger btn-sm" onclick="this.parentElement.remove()"><i class="fas fa-trash"></i></button>`;
    if (targetElement) container.insertBefore(row, targetElement.nextSibling);
    else container.appendChild(row);
    return row;
}
...
function setupProductManagerDragDrop(container) {
    ...
    container.addEventListener('dragstart', e => { draggingElement = e.target; setTimeout(() => e.target.classList.add('dragging'), 0); });
    container.addEventListener('dragend', e => { if (draggingElement) { draggingElement.classList.remove('dragging'); draggingElement = null; } });
...
function addNewQuestionUI() {
    if (currentManagerView !== 'active') { alert("Yeni soru eklemek için 'Aktif Sorular' görünümünde olmalısınız."); return; }
    const managerList = document.getElementById('manager-list');
    ...
    const itemDiv = document.createElement('div');
    itemDiv.className = 'manager-item'; itemDiv.style.backgroundColor = '#dcfce7'; itemDiv.dataset.id = newId;
    itemDiv.innerHTML = `
        <div class="manager-item-grid">
        ...
        <div><label>Statik Maddeler</label><div class="editor-toolbar"><button onclick="formatText(this, 'bold')"><i class="fas fa-bold"></i></button><button onclick="formatText(this, 'italic')"><i class="fas fa-italic"></i></button><button onclick="formatText(this, 'underline')"><i class="fas fa-underline"></i></button><button onclick="formatText(this, 'link')"><i class="fas fa-link"></i></button></div><div class="editable-textarea" contenteditable="true"></div></div>
        <div class="special-manager-container"></div>
        <div class="manager-item-footer"><button class="btn-sm" onclick="this.parentElement.parentElement.remove()">İptal Et</button></div>`;
    managerList.appendChild(itemDiv);
...
function restoreAllArchivedQuestions() {
    ...
    if (itemsToRestore.length === 0) { alert("Aktif edilecek arşivlenmiş soru bulunamadı."); return; }
    if (confirm(`Arşivdeki ${itemsToRestore.length} sorunun tümünü aktif hale getirmek istediğinizden emin misiniz?`)) {
        itemsToRestore.forEach(item => { if (item.querySelector('.archive-checkbox')) item.querySelector('.archive-checkbox').checked = false; });
        filterManagerView();
        alert("Arşivlenmiş tüm sorular aktif hale getirildi. 'Kaydet' butonuna tıklayın.");
    }
}
...
function deleteAllArchivedQuestions() {
    ...
    if (itemsToDelete.length === 0) { alert("Silinecek arşivlenmiş soru bulunamadı."); return; }
    if (confirm(`Arşivdeki ${itemsToDelete.length} sorunun tümünü kalıcı olarak silmek istediğinizden emin misiniz?`)) {
        itemsToDelete.forEach(item => { item.style.transition = 'opacity 0.5s ease'; item.style.opacity = '0'; setTimeout(() => { item.style.display = 'none'; item.classList.add('to-be-deleted'); }, 500); });
        document.getElementById('delete-all-archived-btn').disabled = true;
        alert("Arşivlenmiş sorular silinmek üzere işaretlendi. 'Kaydet' butonuna tıklayın.");
    }
}
...
async function deleteAllAnswersForQuestion(questionId) {
    ...
    if (!confirm(`DİKKAT! FiDe ${questionId} ("${questionTitle}") sorusuna ait TÜM cevapları BÜTÜN raporlardan kalıcı olarak silmek istediğinizden emin misiniz?`)) { alert("İşlem iptal edildi."); return; }
    if (!auth.currentUser || !database) { alert("Bu işlem için bulut bağlantısı gereklidir."); return; }
    const loadingOverlay = document.getElementById('loading-overlay');
    loadingOverlay.style.display = 'flex';
    try {
        const reportsRef = database.ref('allFideReports');
        const snapshot = await reportsRef.once('value');
        if (snapshot.exists()) {
            let allCloudReports = snapshot.val(); let updates = {};
            for (const storeKey in allCloudReports) { if (allCloudReports[storeKey]?.data?.questions_status?.[questionId]) updates[`${storeKey}/data/questions_status/${questionId}`] = null; }
            if (Object.keys(updates).length > 0) await reportsRef.update(updates);
        }
        alert(`İşlem tamamlandı! FiDe ${questionId} sorusuna ait tüm cevaplar silindi.`);
    } catch (error) { console.error("Cevapları silerken hata oluştu:", error); alert("Bir hata oluştu!"); } 
    finally { loadingOverlay.style.display = 'none'; }
}

async function saveQuestions() {
    if (!auth.currentUser || !database) { alert("Değişiklikleri kaydetmek için giriş yapın."); return; }
    ...
    if (activeProductManager && activeProductManager.offsetParent !== null) {
        ...
        rows.forEach(row => {
            if (row.dataset.type === 'category') { const name = row.querySelector('input').value.trim(); if (name) newProductList.push({ type: 'header', name }); } 
            else if (row.dataset.type === 'product') { const code = row.querySelector('.product-code').value.trim(); const name = row.querySelector('.product-name').value.trim(); if (code && name) newProductList.push({ code, name }); }
        });
    } else { productList.forEach(p => newProductList.push(p)); }
    const newQuestions = []; const ids = new Set();
    const items = document.querySelectorAll('#manager-list .manager-item');
    for (const item of items) {
        ...
        if (type === 'pop_system') {
            const popInput = item.querySelector('.pop-codes-input'); const expiredInput = item.querySelector('.expired-pop-codes-input');
            ...
            } else { const o = fideQuestions.find(q => q.id === id); newQuestion.popCodes = o ? o.popCodes : []; newQuestion.expiredCodes = o ? o.expiredCodes : []; }
        }
    ...
    try {
        ...
        window.location.reload();
    } catch (error) { console.error("Kaydetme hatası:", error); alert("HATA: Değişiklikler kaydedilemedi."); } 
    finally { loadingOverlay.style.display = 'none'; }
}
