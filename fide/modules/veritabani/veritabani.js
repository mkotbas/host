// Bu dosya, Veritabanı Bakım Modülü'nün tüm işlevselliğini içerir.
// Kodların çakışmasını önlemek için bir IIFE (Immediately Invoked Function Expression) yapısı kullanılmıştır.
(() => {
    // Firebase veritabanı bağlantısını al
    const database = firebase.database();

    // --- MODAL (AÇILIR PENCERE) YÖNETİM FONKSİYONLARI ---
    ...
    function backupReminder() {
        return confirm("ÖNEMLİ UYARI:\n\nBu işlem veritabanında kalıcı değişiklikler yapacaktır. İşleme başlamadan önce ana uygulamadaki 'Raporları Yedekle' butonunu kullanarak verilerinizin tamamını yedeklemeniz şiddetle tavsiye edilir.\n\nYedek aldınız mı veya bu riski kabul ederek devam etmek istiyor musunuz?");
    }


    // --- KALINTI RAPOR (ORPHAN) ANALİZİ FONKSİYONLARI ---
    ...
    // --- BOZUK RAPOR ANALİZİ FONKSİYONLARI ---
    ...
    // --- VERİ TUTARLILIĞI KONTROLÜ FONKSİYONU ---
    ...
    // --- GEREKSİZ ALAN TEMİZLEYİCİ FONKSİYONLARI ---
    ...
    // --- MODÜL İÇİ OLAY DİNLEYİCİLERİ (EVENT LISTENERS) ---
    ...
    // Modal içerisindeki dinamik butonlar için olayları global scope'a ekleme
    // Bu, onclick="" ile çağrılan fonksiyonların çalışmasını sağlar.
    window.hideModal = hideModal;
    window.deleteSelectedOrphans = deleteSelectedOrphans;
    window.deleteSelectedCorruptReports = deleteSelectedCorruptReports;
    window.cleanObsoleteField = cleanObsoleteField;
