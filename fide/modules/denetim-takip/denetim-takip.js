...
function applyDataFilterAndRunDashboard(viewId) {
    ...
    filteredReports.forEach(record => {
        ...
        if (!yearlyCodesMap.has(storeCode)) {
            yearlyCodesMap.set(storeCode, { code: storeCode, timestamp: reportDate.getTime() });
        }
    ...
    auditedStoreCodesCurrentYear = Array.from(yearlyCodesMap.values())
        .filter(audit => !geriAlinanBayiKodlariYil.has(audit.code));
...
function updateUILabels() {
    ...
    document.getElementById('stat-completed-label').textContent = isYearly ? "Bu Yıl Denetlenen" : "Bu Ay Denetlenen";
    document.getElementById('list-remaining-label').textContent = isYearly ? "Bu Yıl Denetlenecek Bayiler" : "Denetlenecek Bayiler";
    document.getElementById('list-completed-label').textContent = isYearly ? "Bu Yıl Denetlenenler" : `${currentMonthName} Ayı Denetlenenler`;
}
...
function calculateAndDisplayDashboard() {
    ...
    document.getElementById('audited-stores-count').textContent = auditedCount;
...
function renderRemainingStores(filteredStores) {
    ...
    const isYearly = currentPeriod === 'yillik';
    // Aktif periyoda göre denetlenen kodlarını seç
    const auditedCodes = isYearly 
        ? auditedStoreCodesCurrentYear.map(audit => audit.code)
    ...
    if (allRemainingStores.length === 0) {
        ...
        const msg = isYearly ? "Seçili kriterlere uygun, bu yıl denetlenmemiş bayi bulunamadı." : "Seçili kriterlere uygun, bu ay denetlenmemiş bayi bulunamadı.";
        container.innerHTML = `<p class="empty-list-message">${msg}</p>`;
        return;
    }
    ...
    Object.keys(storesByRegion).sort().forEach(region => {
        ...
        const periodLabel = isYearly ? "Bu Yıl" : "Bu Ay";
        let regionHtml = `<div class="region-container"><div class="region-header"><span>${region} (${periodLabel}: ${auditedInRegion}/${totalInRegion})</span></div><div class="progress-bar"><div class="progress-bar-fill" style="width: ${progress.toFixed(2)}%;">${progress.toFixed(0)}%</div></div><ul class="store-list">`;
...
function renderAuditedStores() {
    ...
    if (activeAudits.length === 0) { 
        const msg = isYearly ? "Bu yıl bu görünüm için denetim yapılmadı veya yapılanlar geri alındı." : "Bu ay bu görünüm için denetim yapılmadı veya yapılanlar geri alındı.";
        container.innerHTML = `<p class="empty-list-message">${msg}</p>`;
        return;
    }
    
    const auditedStoresDetails = activeAudits
        .map(audit => {
