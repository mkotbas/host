...
async function loadSettings(uid) {
    ...
    if (typeof auth !== 'undefined' && auth.currentUser && typeof database !== 'undefined') {
        try {
            ...
            console.error("Ayarlar buluttan okunamadı:", error);
        }
    }
...
async function loadStoreList(uid) {
    ...
    if (typeof auth !== 'undefined' && auth.currentUser && typeof database !== 'undefined') {
        try {
            ...
            console.error("Bayi listesi buluttan okunamadı:", error);
        }
    }
    ...
    if (storeData && storeData.stores && storeData.timestamp > sixMonthsAgo) {
        ...
        if (storeData && typeof database !== 'undefined') {
             database.ref(userStoreListPath).remove(); // Süresi geçmiş veriyi buluttan sil
        }
...
async function loadAuditedStoresData(uid) {
    ...
    if (typeof auth !== 'undefined' && auth.currentUser && typeof database !== 'undefined') {
        try {
            ...
            console.error("Denetlenen bayi verileri buluttan okunamadı:", error);
        }
    }
...
// GÜNCELLENDİ: Sadece buluttan okuma ve yazma yapıyor
...
// GÜNCELLENDİ: Sadece buluta yazma yapıyor
...
// GÜNCELLENDİ: Sadece buluttan silme yapıyor
