...
class WhatsAppBayiMesajlasma:
    def __init__(self):
        ...
        self.sehir_mesaj_dosyasi = self.dosya_dizini / 'sehir.txt'
        ...
        hedef_gun = 13
        kalan = hedef_gun - bugun
        kalan_gun_sayisi = max(0, kalan)
        self.gun_metni = f"*{kalan_gun_sayisi} gün*"

        self.mesaj_icerik = ""
        if self.mesaj_dosyasi.exists():
            ham_metin = self.mesaj_dosyasi.read_text(encoding='utf-8').strip()
            self.mesaj_icerik = re.sub(r'\*\d+\s+gün\*', self.gun_metni, ham_metin)
    ...
    def _gonderim_islemi(self, bayi_kodlari_listesi, telefonlar_df, baslik_metni="", mesaj_override=None, mod="normal"):
        ...
        # --- ÖNİZLEME BÖLÜMÜ ---
        print("\n" + "="*50)
        print("GÖNDERİLECEK MESAJ ÖNİZLEMESİ")
        print("="*50)
        
        ornek_kod = "ORNEK123"
        if mod == "ozel_kodlu":
            onizleme = f"Merhaba,\n\n*{ornek_kod}* mağaza koduna ait mağaza/mağazalarınız için;\n\nAşağıdaki ürün gruplarının fotoğraflarını WhatsApp üzerinden iletir misiniz?\n\n{mesaj_override}"
        elif mod == "ozel_sade":
            onizleme = mesaj_override
        else:
            mesaj_govdesi = mesaj_override if mesaj_override else self.mesaj_icerik
            onizleme = f"Merhaba,\n\n*{ornek_kod}* mağaza koduna ait dijital denetim anketiniz ulaşmamıştır.\n\n{mesaj_govdesi}"
        
        print(onizleme)
        print("="*50 + "\n")

        if input("Yukarıdaki mesaj formatını onaylıyor musunuz? Devam edilsin mi? (E/H): ").lower() != 'e': 
    ...
    def calistir(self):
        while True:
            ...
            if secim == '1':
                ...
                if kodlar:
                    if self.sehir_mesaj_dosyasi.exists():
                        ham_sehir_metni = self.sehir_mesaj_dosyasi.read_text(encoding='utf-8').strip()
                        sehir_mesaj_icerik = re.sub(r'\*\d+\s+gün\*', self.gun_metni, ham_sehir_metni)
                        ...
                        if not yeni_gonderilecek:
                            ...
                            # MOD BURADA 'ozel_sade' OLARAK GÜNCELLENDİ: ARTIK OTOMATİK BAŞLIK EKLEMEZ
                            self._gonderim_islemi(yeni_gonderilecek, tels, "Şehir Filtreli Anket Gönderimi", mesaj_override=sehir_mesaj_icerik, mod="ozel_sade")
                    else:
                        print(f"\n[!] Hata: sehir.txt dosyası bulunamadı.")
                        print(f"Lütfen şu konumu kontrol edin: {self.sehir_mesaj_dosyasi.absolute()}")
