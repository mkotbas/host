...
class WhatsAppBayiMesajlasma:
    ...
    def _veri_yukle(self, excel_yolu):
        ...
        # Sayfa ismini kontrol ederek yükleme yap (Öncelik: Tüm Bayiler sayfası)
        try:
    ...
    def calistir(self):
        while True:
            ...
            elif secim == '6': # Şehir Filtreli Anket Gönder (Tüm Bayiler.xlsx)
                kodlar, tels, sehirler = self._veri_yukle(self.tum_bayiler_yolu)
                if kodlar:
                    # Şehir listesini tüm kodlar üzerinden göster
                    secilen_bayiler = self._sehir_secimi_yap(kodlar, sehirler)
                    # Seçilen bayiler içinden sadece gönderilmeyenleri filtrele
            ...
            elif secim == '7': # Kayıtları sıfırla
